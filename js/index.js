...
async function getWeather(location) {
  const response = await fetch(`${baseUrl}?key=${apiKey}&q=${location}&days=4`);
  ...
  for (let [index, day] of days.entries()) {
    ...
    cardsHTML += `
      ...
      <div class="card-header">
        ...
        <div class="time">${date.getHours()}:${date.getMinutes()} ${
      ...
      <div class="card-data">
        <ul class="left-column">
          <li>Real Feel: <span class="real-feel">${
            day.hour[date.getHours()].feelslike_c
          }°C</span></li>
          <li>Wind: <span class="wind">${
            day.hour[date.getHours()].wind_kph
          } K/h</span></li>
          <li>Pressure: <span class="pressure">${
            day.hour[date.getHours()].pressure_mb
          }Mb</span></li>
          <li>Humidity: <span class="humidity">${
            day.hour[date.getHours()].humidity
          }%</span></li>
        </ul>
...
`;
  ...
  document.body.style.backgroundImage = `url(${cityInfo.urls.regular})`;
